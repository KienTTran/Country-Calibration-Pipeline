...
def pbs_counts_qselect(user: str, jobname_target: str) -> tuple[int, int]:
    total_out = subprocess.run(
        ["qselect", "-u", user, "-N", jobname_target],
        capture_output=True, text=True
    )
    total = sum(1 for x in total_out.stdout.splitlines() if x.strip())
...
def submit_and_wait_pbs(cmds, country_code, logger,
    ...
    base_dir = os.path.join("jobs", country_code, "calibration")
    ...
    while True:
        running, ended, total = pbs_counts_qselect(user, jobname_target)
    
        if total == 0:
            break
    
        logger.info(f"Queued jobs {jobname_target}: Running (R): {running} | Finished (E): {ended} | Total jobs (all states): {total}")
